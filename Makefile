...
push:
	...
	git tag v$(VERSION)
	git push --tags
	gh release create -R hughbien/tick -t v$(VERSION) v$(VERSION) ./tick-linux-amd64

install: build
	cp tick $(INSTALL_BIN)

clean:
	rm -rf tick tick-linux-amd64 tick.dwarf
